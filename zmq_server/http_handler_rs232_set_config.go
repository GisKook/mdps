...
func Rs232SetConfigHandler(w http.ResponseWriter, r *http.Request) {
	...
	if rs232_set_cnfig.Plc_id == nil ||
		...
		rs232_set_cnfig.SerialPort == nil ||
		rs232_set_cnfig.StartBit == nil ||
		rs232_set_cnfig.EndBit == nil ||
		rs232_set_cnfig.DataBit == nil ||
		rs232_set_cnfig.Check == nil ||
		rs232_set_cnfig.BaudRate == nil {
	...
	req := &Report.ControlCommand{
		...
		Paras: []*Report.Param{
			&Report.Param{
				...
				Npara: uint64(*rs232_set_cnfig.SerialPort),
			},
			&Report.Param{
				Type:  Report.Param_UINT8,
				Npara: uint64(*rs232_set_cnfig.StartBit),
			},
			&Report.Param{
				Type:  Report.Param_UINT8,
				Npara: uint64(*rs232_set_cnfig.EndBit),
			},
			&Report.Param{
				Type:  Report.Param_UINT8,
				Npara: uint64(*rs232_set_cnfig.DataBit),
			},
			&Report.Param{
				Type:  Report.Param_UINT8,
				Npara: uint64(*rs232_set_cnfig.Check),
			},
			&Report.Param{
				Type:  Report.Param_UINT32,
				Npara: uint64(*rs232_set_cnfig.BaudRate),
			},
		},
	}
